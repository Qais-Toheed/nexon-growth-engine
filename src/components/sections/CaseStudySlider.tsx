...
const catTokens: Record<string, { bg: string; text: string; border: string; glow: string; grad: [string, string]; bar: string }> = {
  Web:       { bg: "hsl(var(--primary)/0.08)", text: "hsl(var(--primary))", border: "hsl(var(--primary)/0.22)", glow: "hsl(214 100% 50% / 0.14)", grad: ["hsl(214 100% 50% / 0.08)", "hsl(188 97% 44% / 0.04)"], bar: "#0080FF" },
  Ecommerce: { bg: "hsl(var(--cyan)/0.08)",    text: "hsl(var(--cyan))",    border: "hsl(var(--cyan)/0.22)",    glow: "hsl(188 97% 44% / 0.13)", grad: ["hsl(188 97% 44% / 0.08)", "hsl(255 82% 62% / 0.04)"], bar: "#09BDD6" },
  Marketing: { bg: "hsl(var(--violet)/0.08)",  text: "hsl(var(--violet))", border: "hsl(var(--violet)/0.22)",  glow: "hsl(255 82% 62% / 0.13)", grad: ["hsl(255 82% 62% / 0.08)", "hsl(214 100% 50% / 0.04)"], bar: "#7A52F4" },
  AI:        { bg: "hsl(var(--primary)/0.08)", text: "hsl(var(--primary))", border: "hsl(var(--primary)/0.22)", glow: "hsl(214 100% 50% / 0.14)", grad: ["hsl(214 100% 50% / 0.08)", "hsl(255 82% 62% / 0.04)"], bar: "#0080FF" },
  App:       { bg: "hsl(var(--cyan)/0.08)",    text: "hsl(var(--cyan))",    border: "hsl(var(--cyan)/0.22)",    glow: "hsl(188 97% 44% / 0.13)", grad: ["hsl(188 97% 44% / 0.07)", "hsl(214 100% 50% / 0.03)"], bar: "#09BDD6" },
};

function MockupFrame({ cat, title }: { cat: typeof catTokens[string]; title: string }) {
  return (
    <div className="absolute inset-6 rounded-2xl overflow-hidden shadow-xl"
      ...
      <div className="h-16 flex items-center px-4 gap-3"
        ...
        <div className="w-8 h-8 rounded-xl flex-shrink-0" style={{ background: cat.bar, opacity: 0.9 }} />
        <div className="space-y-1 flex-1">
          <div className="h-2.5 w-2/3 rounded-full" style={{ background: cat.bar, opacity: 0.8 }} />
          <div className="h-1.5 w-1/2 rounded-full" style={{ background: "hsl(var(--border))" }} />
        </div>
      </div>
      {/* Content skeleton */}
      <div className="p-3 space-y-2">
        <div className="grid grid-cols-3 gap-2">
          {[1,2,3].map(n => (
            <div key={n} className="h-10 rounded-xl" style={{ background: "hsl(var(--surface))", border: "1px solid hsl(var(--border))" }} />
          ))}
        </div>
        ...
        <div className="h-8 w-2/5 rounded-xl mt-1"
          style={{ background: cat.bar, opacity: 0.18 }} />
      </div>
    </div>
  );
}
          ...
          <div className="hidden sm:flex items-center gap-3">
            {[{ fn: scrollPrev, icon: ChevronLeft }, { fn: scrollNext, icon: ChevronRight }].map(({ fn, icon: Icon }, idx) => (
              <button key={idx} onClick={fn}
                ...
                style={{ background: "hsl(220 20% 100%)", border: "1px solid hsl(var(--border))", boxShadow: "0 2px 12px hsl(220 30% 10% / 0.05)", color: "hsl(var(--muted-foreground))" }}
                onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = "hsl(var(--primary)/0.30)"; el.style.color = "hsl(var(--primary))"; el.style.boxShadow = "0 4px 20px hsl(214 100% 50% / 0.15)"; }}
                onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = "hsl(var(--border))"; el.style.color = "hsl(var(--muted-foreground))"; el.style.boxShadow = "0 2px 12px hsl(220 30% 10% / 0.05)"; }}
        ...
        {caseStudies[0] && (() => {
          ...
          return (
            <motion.div
              ...
              style={{ background: `linear-gradient(135deg, ${cat.grad[0]}, ${cat.grad[1]})`, border: `1px solid ${cat.border}`, boxShadow: `0 24px 80px ${cat.glow}` }}
            >
              <div className="grid grid-cols-1 lg:grid-cols-2">
                {/* Visual panel */}
                <div className="relative h-[300px] lg:h-auto min-h-[300px] overflow-hidden"
                  style={{ background: `linear-gradient(135deg, ${cat.grad[0]}, transparent)` }}>
                  <MockupFrame cat={cat} title={featured.title} />
                  <div className="absolute top-4 left-4">
                    <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest"
                      style={{ background: cat.bar, color: "white" }}>Featured</span>
                  </div>
                  {/* Decorative floating elements */}
                  <div className="absolute bottom-4 right-4 w-16 h-16 rounded-2xl opacity-20"
                    style={{ background: cat.bar, animation: "float 5s ease-in-out infinite" }} />
                </div>
                ...
                <div className="p-10 lg:p-12 flex flex-col justify-center">
                  ...
                  <h3 className="text-2xl lg:text-3xl font-black mb-6 leading-snug" style={{ color: "hsl(var(--foreground))" }}>
                    {featured.title}
                  </h3>
                  <div className="space-y-4 mb-8">
                    {[{ label: "Challenge", text: featured.challenge }, { label: "Outcome", text: featured.outcome }].map(({ label, text }) => (
                  ...
                  <Link to="/contact" className="inline-flex items-center gap-2 font-bold text-sm group self-start" style={{ color: cat.text }}>
          ...
          <div className="flex gap-5">
            {caseStudies.slice(1).map((study, i) => {
              ...
              return (
                ...
                >
                  <div className="h-full flex flex-col rounded-2xl overflow-hidden transition-all duration-400"
                    style={{ background: "hsl(220 20% 100%)", border: "1px solid hsl(var(--border))", boxShadow: "0 2px 16px hsl(220 30% 10% / 0.05)" }}
                    onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = cat.border; el.style.boxShadow = `0 16px 60px ${cat.glow}`; el.style.transform = "translateY(-4px)"; }}
                  ...
                  >
                    {/* Visual header with mini mockup */}
                    <div className="h-48 relative overflow-hidden"
                      style={{ background: `linear-gradient(135deg, ${cat.grad[0]}, ${cat.grad[1]})` }}>
                      <div className="absolute inset-4 rounded-xl overflow-hidden"
                        style={{ background: "hsl(220 20% 100% / 0.90)", border: "1px solid hsl(var(--border))" }}>
                        ...
                        <div className="p-3 space-y-1.5">
                          <div className="h-2.5 w-3/5 rounded-full" style={{ background: cat.bar, opacity: 0.75 }} />
                          ...
                          <div className="flex gap-2 mt-2">
                            <div className="flex-1 h-8 rounded-lg" style={{ background: "hsl(var(--surface))" }} />
                            <div className="flex-1 h-8 rounded-lg" style={{ background: "hsl(var(--surface))" }} />
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="flex-1 flex flex-col p-6">
                      ...
                      <h3 className="text-base font-bold mb-4 leading-snug flex-1" style={{ color: "hsl(var(--foreground))" }}>
                        {study.title}
                      </h3>
                      <div className="pl-3.5 mb-5" style={{ borderLeft: `2px solid ${cat.text}` }}>
                        <span className="text-[9px] font-black uppercase tracking-widest block mb-0.5" style={{ color: cat.text }}>Outcome</span>
                        <p className="text-xs leading-relaxed line-clamp-2" style={{ color: "hsl(var(--muted-foreground))" }}>
                          {study.outcome}
                        </p>
                      </div>
                      <Link to="/contact" className="inline-flex items-center gap-1.5 text-xs font-bold group" style={{ color: cat.text }}>
                        Similar results?
